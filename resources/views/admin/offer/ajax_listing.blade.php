...
<tr>
   <td>
    ...
    @if($value->vImage !='' && file_exists(public_path()."/uploads/offer/".$value->vImage))
    <a href="{{asset('/uploads/offer/'.$value->vImage)}}" target="_blank">
    <img style="width: 90px;border-radius: 10px;"  src="{{asset('uploads/offer/'.$value->vImage)}}"></a>
    @else
   ...
   @if($value->eType != null && $value->eType == "Offers")
   <td class="text-center">
      @if(!empty($value->vTitle) && $value->vTitle !=  null  && $value->vPercentage !=  null && $value->vStartDate !=  null && $value->vEndDate ==  null)
      {{$value->vPercentage." ".$value->vTitle." ".$value->vStartDate}}
      @elseif(!empty($value->vTitle) && $value->vTitle !=  null  && $value->vPercentage !=  null && $value->vStartDate !=  null && $value->vEndDate !=  null)
      {{$value->vPercentage." ".$value->vTitle." ".$value->vStartDate." to ".$value->vEndDate}}
      @else
      {{ "N/A" }}
      @endif
   </td>
   ...
   <td>
      ...
      <div class="bottom-btn text-center">
         ...
         <a href="{{url('admin/offer/edit',$value->vUniqueCode)}}" data-toggle="tooltip_edit" title="Edit Data" data-id="{{$value->vUniqueCode}}" class="btn btn-outline-primary btn-sm me-1"><i class='bx bx-edit-alt pt-1 pb-1'></i></a>
         @endif
