...
@section('title', 'Sub Admin Listing- '.env('APP_NAME'))
@section('content')
<!-- Main content -->
<div class="container-xxl flex-grow-1 container-p-y">
  <div class="d-flex justify-content-between flex-wrap gap-3 top-input-space"> 
    ...
    <div class="d-flex flex-wrap right-side-top">
        ...
        <div class="space">
                 <a href="{{route('admin.subadmin.add')}}"  data-toggle="tooltip_add" title="Add New Sub Admin" class="btn btn-primary">+ Add</a>
        </div>
        <div class="space">
...
<script type="text/javascript">
    ...
    { 
        ...
        setTimeout(function() {
        $.ajax({
           url: "{{url('admin/subadmin/ajax_listing')}}",
           type: "POST",
...
$(".dawnload_excel").click(function(){    
        ...
        $('.loading').show();
          setTimeout(function(){
            $.ajax({
               url: "{{url('admin/subadmin/admin_data_listing_excel')}}",
               type: "POST",
               headers:{'X-CSRF-TOKEN':$('meta[name="csrf-token"]').attr('content')},
               data: {eStatuschange:eStatuschange,keyword:keyword, eStatus_search:eStatus_search,eStatus:eStatus,eDeleted:eDeleted},
                ...
                { 
                  ...
                  if(data.length != 0)
                  {
                       ...
                       window.location.href = "{{url('admin/subadmin')}}";
                  }
