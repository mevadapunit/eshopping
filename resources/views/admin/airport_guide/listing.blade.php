...
@section('title', 'Airport Guide Listing- '.env('APP_NAME'))
@section('content')
...
<div class="d-flex gap-3 top-input-space">
        ...
        <div class="d-flex flex-wrap right-side-top">
            ...
            <div class="space">
                     <a href="{{url('admin/airportguide/add')}}" class="btn btn-primary">+ Add</a>
            </div>
            @endif
        </div>
    </div>
               ...
               <table class="table table-bordered">
                  <thead>
                     <tr>
                        ...
                        <th class="space-w"> <select class="form-select" name="eCity" id="eCity">
...
<script type="text/javascript">
    ...
    function filter(vUniqueCode, vAction, vPages)
    {
        ...
        setTimeout(function() {
        $.ajax({
           url: "{{url('admin/airportguide/ajax_listing')}}",
           type: "POST",
           headers:{'X-CSRF-TOKEN':$('meta[name="csrf-token"]').attr('content')},
           data: {vUniqueCode:vUniqueCode,vAction:vAction,eCity:eCity,eMenu:eMenu,pages:pages, vTitle:vTitle, eStatus_search:eStatus_search,eStatus:eStatus,eDeleted:eDeleted,eFeature_search:eFeature_search},
    ...
    $(document).on("change", "#eCity", function() {
        filter("", "search", "");
    });
