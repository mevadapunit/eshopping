...
<tr>
   <td>
    ...
    @if($value->vFile !='' && file_exists(public_path()."/uploads/how_to_use/".$value->vFile))
    <a href="{{asset('/uploads/how_to_use/'.$value->vFile)}}" target="_blank">
    <video style="width: 90px;border-radius: 10px;" controls src="{{asset('uploads/how_to_use/'.$value->vFile)}}"></a>
    @else
   ...
   <td>
      ...
      <div class="bottom-btn text-center">
         ...
         <a href="{{route('admin.how_to_use.edit',$value->vUniqueCode)}}" data-toggle="tooltip_edit" title="Edit Data" data-id="{{$value->vUniqueCode}}" class="btn btn-outline-primary btn-sm me-1"><i class='bx bx-edit-alt pt-1 pb-1'></i></a>
         @endif
