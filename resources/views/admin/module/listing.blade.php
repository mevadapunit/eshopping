...
@section('title', 'Module Listing- '.env('APP_NAME'))
@section('content')
                  ...
                  <thead>
                     <tr>
                        ...
                        <th style="width:147px;"><select id="iRoleId" name="iRoleId" class="form-select selectedrole">
                         ...
                         <th style="width:147px;"><select id="iMenuId" name="iMenuId" class="form-select selectedmenu">
                               <option value="">Menu</option>
                               @foreach($menues as $value)
                               <option value="{{$value->iMenuId}}">{{$value->vMenu}}</option>
                               @endforeach
                           </select>
                         </th>
...
<script type="text/javascript">
    ...
    function filter(vUniqueCode, vAction, vPages, iRoleId , iMenuId)
    {
        ...
        setTimeout(function() {
        $.ajax({
           url: "{{url('admin/module/ajax_listing')}}",
           type: "POST",
           headers:{'X-CSRF-TOKEN':$('meta[name="csrf-token"]').attr('content')},
           data: {vUniqueCode:vUniqueCode, vAction:vAction, pages:pages,vController:vController,iRoleId:iRoleId,iMenuId:iMenuId,vModule:vModule, eStatus_search:eStatus_search,eStatus:eStatus,eDeleted:eDeleted,eFeature_search:eFeature_search},
    ...
    $(document).ready(function() {
     ...
     $(document).on("change", ".selectedrole", function() {
        ...
        var iMenuId    = $('#iMenuId :selected').val();
         filter("", "search", "",iRoleId,iMenuId);
    });

     $(document).on("change", ".selectedmenu", function() {
        var iMenuId    = $(this).val();
    ...
    $(document).on("change", "#eDeleted", function() {
     ...
     $(document).on('click', '.ajax_page', function() {
        ...
        var iRoleId    = $('#iRoleId :selected').val();
        var iMenuId    = $('#iMenuId :selected').val();
        filter("", "search", vPages,iRoleId,iMenuId);
