...
<tr>
   ...
   <td class="text-center">
    @if($value->vLogo !='' && file_exists(public_path()."/uploads/banner/".$value->vLogo))
    <a href="{{asset('/uploads/banner/'.$value->vLogo)}}" target="_blank">
    <img style="width: 90px;border-radius: 10px;"  src="{{asset('uploads/banner/'.$value->vLogo)}}"></a>
    @else
    ...
    <td class="text-center">
    @if($value->vImage !='' && file_exists(public_path()."/uploads/banner/".$value->vImage))
    <a href="{{asset('/uploads/banner/'.$value->vImage)}}" target="_blank">
    <img style="width: 90px;border-radius: 10px;"  src="{{asset('uploads/banner/'.$value->vImage)}}"></a>
    @else
    <div class="text-center">
     <img style="width: 90px;border-radius: 10px;" src="{{ asset('admin/assets/img/no_image.png') }}" alt="No photo">
    </div>
   ...
   <td>
      ...
      <div class="bottom-btn text-center">
         ...
         <a href="{{url('admin/banner/edit',$value->vUniqueCode)}}" data-toggle="tooltip_edit" title="Edit Data" data-id="{{$value->vUniqueCode}}" class="btn btn-outline-primary btn-sm me-1"><i class='bx bx-edit-alt pt-1 pb-1'></i></a>
         @endif
