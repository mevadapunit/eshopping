...
<tr>
   ...
   <td class="text-left">
      @if(!empty($value->vImmigrationStatus))
      {{$value->vImmigrationStatus}}
      @else
      {{ "N/A" }}
      @endif
   </td>
   ...
   <td>
      ...
      <div class="bottom-btn text-center">
         ...
         <a href="{{url('admin/immigrationstatus/edit',$value->vUniqueCode)}}" data-toggle="tooltip_edit" title="Edit Data" data-id="{{$value->vUniqueCode}}" class="btn btn-outline-primary btn-sm me-1"><i class='bx bx-edit-alt pt-1 pb-1'></i></a>
         @endif
