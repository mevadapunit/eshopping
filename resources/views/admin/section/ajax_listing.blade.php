...
<tr>
   <td>
    ...
    @if($value->vImage !='' && file_exists(public_path()."/uploads/section/".$value->vImage))
    <a href="{{asset('/uploads/section/'.$value->vImage)}}" target="_blank">
    <img style="width: 90px;border-radius: 10px;"  src="{{asset('uploads/section/'.$value->vImage)}}"></a>
    @else
   ...
   <td>
      ...
      <div class="bottom-btn text-center">
         ...
         <a href="{{url('admin/section/edit',$value->vUniqueCode)}}" data-toggle="tooltip_edit" title="Edit Data" data-id="{{$value->vUniqueCode}}" class="btn btn-outline-primary btn-sm me-1"><i class='bx bx-edit-alt pt-1 pb-1'></i></a>
         @endif
