...
@section('title', 'Driver Listing- '.env('APP_NAME'))
@section('content')
<!-- Main content -->
<div class="container-xxl flex-grow-1 container-p-y">
  <div class="d-flex justify-content-between flex-wrap gap-3 top-input-space"> 
    ...
    <div class="d-flex flex-wrap right-side-top">
        ...
        <div class="space">
                 <a href="{{url('admin/driver/add')}}" data-toggle="tooltip_add" title="Add New Driver"   class="btn btn-primary">+ Add</a>
        </div>
        <div class="space">
  ...
  $(document).on('keypress','#vPhone',function(e)
{
    ...
    function filter(vUniqueCode, vAction, vPages,eStatuschange)
    {
        ...
        setTimeout(function() {
        $.ajax({
           url: "{{url('admin/driver/ajax_listing')}}",
           type: "POST",
...
$(".dawnload_excel").click(function(){    
        ...
        setTimeout(function(){
            $.ajax({
               url: "{{url('admin/driver/driver_data_listing_excel')}}",
               type: "POST",
               headers:{'X-CSRF-TOKEN':$('meta[name="csrf-token"]').attr('content')},
