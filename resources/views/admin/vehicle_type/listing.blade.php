...
@section('title', 'Vehicle Type Listing- '.env('APP_NAME'))
@section('content')
...
<div class="d-flex gap-3 top-input-space">
        ...
        <div class="d-flex flex-wrap right-side-top">
            ...
            <div class="space">
                     <a href="{{url('admin/vehicletype/add')}}" class="btn btn-primary">+ Add</a>
            </div>
            @endif
        </div>
    </div>
               ...
               <table class="table table-bordered">
                  <thead>
                     <tr>
                        ...
                        <th class="search-input"><input placeholder="Title" id="vTitle" type="text" name="vTitle" class="search form-control"></th>
                        <th><select id="iVehicleTypeId" name="iVehicleTypeId" class="form-select selectedrole">
                               <option value="">Vehicle Type</option>
                               @foreach($vehicle_type as $value)
                               <option value="{{$value->iVehicleTypeId}}">{{$value->vVehicleType}}</option>
                               @endforeach
                           </select>
                         </th>
...
<script type="text/javascript">
    ...
    function filter(vUniqueCode, vAction, vPages,iVehicleTypeId)
    {
        ...
        setTimeout(function() {
        $.ajax({
           url: "{{url('admin/vehicletype/ajax_listing')}}",
           type: "POST",
           headers:{'X-CSRF-TOKEN':$('meta[name="csrf-token"]').attr('content')},
           data: {vUniqueCode:vUniqueCode, vAction:vAction, pages:pages,vTitle:vTitle,iVehicleTypeId:iVehicleTypeId,eStatus_search:eStatus_search,eStatus:eStatus,eDeleted:eDeleted,eFeature_search:eFeature_search,},
