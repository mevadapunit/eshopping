...
@section('title', 'Rider Listing- '.env('APP_NAME'))
@section('content')
<!-- Main content -->
<div class="container-xxl flex-grow-1 container-p-y">
  <div class="d-flex justify-content-between flex-wrap gap-3 top-input-space"> 
    ...
    <div class="d-flex flex-wrap right-side-top">
        ...
        <div class="space">
                 <a href="{{url('admin/rider/add')}}" data-toggle="tooltip_add" title="Add New Rider" class="btn btn-primary">+ Add</a>
        </div>
         <div class="space">
...
<script type="text/javascript">
    ...
    function filter(vUniqueCode, vAction, vPages,eStatuschange)
    {
        ...
        setTimeout(function() {
        $.ajax({
           url: "{{url('admin/rider/ajax_listing')}}",
           type: "POST",
           headers:{'X-CSRF-TOKEN':$('meta[name="csrf-token"]').attr('content')},
           data: {vUniqueCode:vUniqueCode, vAction:vAction, pages:pages,eStatuschange:eStatuschange,keyword:keyword, eStatus_search:eStatus_search,eStatus:eStatus,eDeleted:eDeleted},
...
$(".dawnload_excel").click(function(){    
        ...
        $('.loading').show();
          setTimeout(function(){
            $.ajax({
               url: "{{url('admin/rider/rider_data_listing_excel')}}",
               type: "POST",
               headers:{'X-CSRF-TOKEN':$('meta[name="csrf-token"]').attr('content')},
               data: {eStatuschange:eStatuschange,keyword:keyword, eStatus_search:eStatus_search,eStatus:eStatus,eDeleted:eDeleted},
                ...
                { 
                  ...
                  if(data.length != 0)
                  {
                       ...
                       window.location.href = "{{url('admin/rider')}}";
                  }
